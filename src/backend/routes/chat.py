...
async def chat(request: Request, body: MessageRequest, session: AsyncSession = Depends(get_session)) -> MessageResponse:
    """Run a single conversational turn through the chatbot agent."""
    logger.info("Chat turn started (thread_id=%s): %r", body.thread_id, body.question)

    agent = request.app.state.agent
    try:
        result = await asyncio.wait_for(agent.ainvoke({"messages": [HumanMessage(content=body.question)]}, context=Context(session=session), config={"configurable": {"thread_id": body.thread_id}}), timeout=Config.CHAT_TIMEOUT_SECONDS)
    except:
    ...
    for m in result["messages"]:
        logger.info("%s -> %r", type(m).__name__, m)
