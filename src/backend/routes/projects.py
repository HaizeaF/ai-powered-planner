...
def create_project(project_create: ProjectCreate, session: Session = Depends(get_session)) -> ProjectRead:
...
def list_projects(session: Session = Depends(get_session)) -> list[ProjectRead]:
    """List all projects."""
    service = ProjectService(session)
    return [_to_read(service, project) for project in service.get_all()]
...
def update_project(project_id: int, project_update: ProjectUpdate, session: Session = Depends(get_session)) -> ProjectRead:
